...
class SPNet(nn.Module):
    def __init__(self, channel=32, ind=50):
        ...
        #self.layer_deps = Res2Net_model(ind)
        self.layer_deps = timm.create_model(model_name="resnet18", pretrained=True, in_chans=3, features_only=True)
        ...
        #  depth-decoders #
        ###############################################
        self.depths_conv_4 = nn.Sequential(BasicConv2d(512, 256, 3, padding=1), self.relu)
        self.depths_conv_3 = nn.Sequential(BasicConv2d(256, 128, 3, padding=1), self.relu)
    ...
    def forward(self, imgs, depths):
        ...
        x4 =  self.depth_upsample_2(self.depths_conv_4( deps_4)) + deps_3
        x3 =  self.depth_upsample_2(self.depths_conv_3( x4))  + deps_2
        x2 =  self.depth_upsample_2(self.depths_conv_2( x3))  + deps_1
        x1 =  self.depth_upsample_2(self.depths_conv_1( x2))  + deps_0
        x0_1 =  self.depths_conv_0(x1)
        x0_2 =  self.depth_upsample_2(x0_1)
        ...
        return rgb_out, dep_out, ful_out, t_out, x0_2
